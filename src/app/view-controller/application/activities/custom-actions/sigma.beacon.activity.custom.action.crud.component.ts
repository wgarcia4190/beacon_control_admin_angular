...
@Component({
	selector: 'sigma-beacon-custom-action-crud',
	encapsulation: ViewEncapsulation.None,
	templateUrl: './sigma.beacon.activity.custom.action.crud.component.html',
	styleUrls: [
...
export class SigmaBeaconCustomActionCrudComponent implements OnInit, AfterViewInit {
	...
	private addNewAttribute(): void {
		this.activity.customAttributes.push({
			'name': '',
			'value': ''
		});
	}

	private removeAttribute(index: number): void {
		this.activity.customAttributes.splice(index, 1);
	}

	private setValueToAttribute(event: any, key: string, index: number): void {
		this.activity.customAttributes[index][key] = event.target.value;
	}
	...
	private saveActivity(): void {
		...
		if (this.validForm.controls['activityName'].valid) {
